'use strict'; // essential functions
...
const DEFAULT_FALLBACK_MESSAGES = ['what do you say?', 'Please make your order clear'];
/**
...
function SaihuBot(config) {
  ...
  this.welcomeMessage = config.welcomeMessage || 'type something to chat with me';
  this.notFoundMessages = config.notFoundMessages || DEFAULT_FALLBACK_MESSAGES;
  this.saveChatLog = config.saveChatLog || true; // provide run, close, send, render function by adapter
...
SaihuBot.prototype = {
  ...
  run: function () {
    this.DEBUG && console.log(`run with ${this.brain.name} brain and ${this.adapter.name} adapter`); // running loop

    this.brain.run(this, this.restore.bind(this), this.brainConfig); // shutdown flow
  ...
  loadSkill: function (skill) {
    ...
    if (!skill || !skill.name || !skill.rule || !skill.action || typeof skill.action !== 'function') {
  ...
  loadAddon: function (addon) {
    ...
    if (!addon || !addon.name || !addon.action || typeof addon.action !== 'function') {
  ...
  hasRightDependency: function (requirements) {
    ...
    if (addons.length !== 0) {
      validAddons = addons.every(name => Object.keys(this.addons).includes(name));
    }

    if (skills.length !== 0) {
      validSkills = skills.every(name => this.responses.map(skill => skill.name).includes(name));
    }
